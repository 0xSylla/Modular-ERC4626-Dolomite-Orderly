...
/// @title VaultCuratorRouterV3
/// @notice Identical to V2 with one addition: `setPositionStatus(vault, positionId, newStatus)`.
///
///         Motivation: V2 has no escape hatch from REBALANCING when the rebalance reopen is
///         infeasible (e.g., vault USDC dropped below the perps reopen floor after the close
///         half completed). The position record stays at REBALANCING forever, the cron retries
///         every 60s and reverts each time, and the only way out is a manual top-up of the vault.
///
///         `setPositionStatus` is a generic operator-callable escape hatch that lets the bot
///         transition a stuck position to any valid status — including UNDEFINED (= delete the
///         record entirely), which is what we use for the floor-stall case. Funds sit idle in
///         the vault for the curator to redeploy via a fresh `definePosition` or for depositors
///         to redeem when the cycle next opens withdrawals.
///
///         Trade-off: the operator can now move any position to any state. Mitigated by:
///           - OPERATOR_ROLE remains the only role that can call it
///           - All transitions are logged via `PositionStatusUpdated` for monitoring
///           - The operator already has significant on-chain power (operatorRebalanceClose,
///             executeOpeningRequest etc.), so this is an incremental delegation.
contract VaultCuratorRouterV3 {
    ...
    // ============ V3 Events (not in Events.sol since they're router-specific) ============

    /// @notice Emitted when the operator forces a position status change. Use this to detect
    /// stuck-state recoveries off-chain.
    ...
    // ============ Auto-Rebalance (V2 addition, unchanged in V3) ============
    ...
    // ============ Status Recovery (V3 addition) ============

    /// @notice Force a position to a new status. Used to recover from stuck states (most
    /// commonly: REBALANCING when reopen is infeasible due to vault USDC < perps floor).
    ///
    /// Pass `Data.PositionStatus.IDLE` to reset the position so the curator can redeploy via
    /// the normal lifecycle (requestOpeningPosition → executeOpeningRequest). Note: this does
    /// NOT delete the record; the curator can also call `removePosition` after this to fully
    /// drop the record if they want to start over from scratch.
    ///
    /// Trade-off: the operator can move any position to any state. This is an incremental
    /// delegation on top of operatorRebalanceClose / executeOpeningRequest etc. All transitions
    /// are emitted via `PositionStatusUpdated` for off-chain monitoring.
