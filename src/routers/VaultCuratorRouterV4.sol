...
/// @title VaultCuratorRouterV4
/// @notice Identical to V3 with one structural addition: `definePosition` now refuses to
///         create a second position with the same `(collateralAsset, perpsAsset)` pair while
///         the existing one is still live (status != IDLE).
///
///         Motivation: a vault has a single per-vault operator EOA, and HL aggregates trades
///         per (user, coin). Two ETH shorts on the same vault collapse into one HL position
///         with one mark/funding/PnL stream — the bot has no way to attribute the aggregate
///         back to individual position IDs. Morpho aggregates the same way per (user, market).
///         The TP/SL algo orders also collide. Enforcing uniqueness here is the cheapest fix.
///
///         An IDLE record (closed, undeployed) is allowed to coexist because it has no
///         on-chain legs and no off-chain state — the interference window is strictly the
///         active lifecycle. Curators can also `removePosition` an IDLE record if they want
///         to fully reset.
contract VaultCuratorRouterV4 {
    ...
    /// @dev V4: refuses a duplicate (collateralAsset, perpsAsset) pair if any existing record
    /// for this vault is currently non-IDLE. The loop bound is `nextPositionId[vault]`, which
    /// is monotonic — skipped slots (deleted via removePosition) are detected by p.vault == 0.
    function definePosition(
        ...
        bytes32 newPerpsHash = keccak256(bytes(perpsAsset));
        uint256 upper = nextPositionId[vault];
        for (uint256 i = 0; i < upper; i++) {
            PositionRecord storage existing = positions[vault][i];
            if (existing.vault == address(0)) continue; // removed
            if (existing.status == Data.PositionStatus.IDLE) continue; // dormant, safe
            if (
                existing.collateralAsset == collateralAsset &&
                keccak256(bytes(existing.perpsAsset)) == newPerpsHash
            ) revert Events.DuplicatePositionPair();
        }
    ...
    function requestOpeningPosition(
        ...
        // V4: also block re-opening if another (same-pair) position is currently live.
        // Mirrors the definePosition guard for the "two IDLE records of the same pair,
        // requestOpeningPosition on both" race.
        bytes32 newPerpsHash = keccak256(bytes(p.perpsAsset));
        address collat = p.collateralAsset;
        uint256 upper = nextPositionId[vault];
        for (uint256 i = 0; i < upper; i++) {
            if (i == positionId) continue;
            PositionRecord storage other = positions[vault][i];
            if (other.vault == address(0)) continue;
            if (other.status == Data.PositionStatus.IDLE) continue;
            if (
                other.collateralAsset == collat &&
                keccak256(bytes(other.perpsAsset)) == newPerpsHash
            ) revert Events.DuplicatePositionPair();
        }
