...
contract DiracHoneyPotV1 is Controller, ERC4626Upgradeable {
        ...
        whenNotPaused
    {
        ...
        borrowAsset.safeIncreaseAllowance(
            address(DEPOSIT_WITHDRAWAL_ROUTER),
            repayAmount
        );
    ...
    /////////////////////////// SETTERS FUNCTIONS //////////////////////////////
    ////////////////////////////////////////////////////////////////////////////
    function setDolomiteIsolationProxy(
        address _proxy
     ...
     * @dev Withdraws all iBGT and USDC to the admin.
     */
    function emergencyWithdraw() external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 ibgtBalance = collateralAsset.balanceOf(address(this));
        uint256 usdcBalance = borrowAsset.balanceOf(address(this));

        if (ibgtBalance > 0) {
            collateralAsset.safeTransfer(msg.sender, ibgtBalance);
        }
        if (usdcBalance > 0) {
            borrowAsset.safeTransfer(msg.sender, usdcBalance);
        }
    }
}
